...
(defun peca-b(posx posy tabuleiro)
  ...
  (cond
    ((equal (check-all posx posy tabuleiro 'peca-b) t)        
     (substituir (car (car (cdddr (peca-casas-ocupadas posx posy 'peca-b))))  (car (cdr (car (cdddr (peca-casas-ocupadas posx posy 'peca-b))))) ;; 4 iterações 
              (substituir (caar (cddr (peca-casas-ocupadas posx posy 'peca-b)))  (car ( cdr (car (cdr (cdr (peca-casas-ocupadas posx posy 'peca-b)))))) ;; Tabuleiro = Função Anterior
...
;11. peca-c-1: Função que recebe dois índices e o tabuleiro e coloca a peça esse na posição de lado no
;tabuleiro tendo como ponto de referência o índice passado como argumento. A função deverá retornar
;NIL caso alguma das casas a serem preenchidas não esteja vazia ou caso a peça esteja a ser colocada
...
(defun peca-c-1(posx posy tabuleiro)
  ...
  (cond
    ((equal (check-all posx posy tabuleiro 'peca-c-1) t)     
     (substituir (car (cdr (car (cdddr (peca-casas-ocupadas posx posy 'peca-c-1))))) (car (car (cdddr (peca-casas-ocupadas posx posy 'peca-c-1)))) ;; 4 iterações 
              (substituir (car ( cdr (car (cdr (cdr (peca-casas-ocupadas posx posy 'peca-c-1))))))(caar (cddr (peca-casas-ocupadas posx posy 'peca-c-1))) ;; Tabuleiro = Função Anterior
...
;;12. peca-c-2: Função que recebe dois índices e o tabuleiro e coloca a peça esse na posição para cima no
;;tabuleiro tendo como ponto de referência o índice passado como argumento. A função deverá retornar
;;NIL caso alguma das casas a serem preenchidas não esteja vazia ou caso a peça esteja a ser colocada
;;fora dos limites do tabuleiro.
(defun peca-c-2(posx posy tabuleiro)
  ...
  (cond
    ((equal (check-all posx posy tabuleiro 'peca-c-2) t)     
     (substituir (car (cdr (car (cdddr (peca-casas-ocupadas posx posy 'peca-c-2))))) (car (car (cdddr (peca-casas-ocupadas posx posy 'peca-c-2)))) ;; 4 iterações 
              (substituir (car ( cdr (car (cdr (cdr (peca-casas-ocupadas posx posy 'peca-c-2))))))(caar (cddr (peca-casas-ocupadas posx posy 'peca-c-2))) ;; Tabuleiro = Função Anterior
