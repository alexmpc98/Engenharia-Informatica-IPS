...
public class Board implements Serializable {
    ...
    /**
     * Este método retorna um array bidimensional com o tabuleiro de jogo.
     *
     * @return Tabuleiro de jogo String Array[][]
     */
    ...
    public boolean checkIfBoardNotEmpty() {
        ...
        for (int row = 0; row < rowCount; row++) {
            for (int col = 0; col < colCount; col++) {
                //System.out.print(" " + this.boardPositions[row][col]);
                if (this.boardPositions[row][col].equals("#")) {
                    return true;
                }
            }
            //System.out.print("\n");
    ...
    /**
     * Este método percorre o tabuleiro e verifica se está cheio, e se sim
     * retorna verdadeira, senão retorna falso.
     *
     ...
     * inserido e verifica se é possivel colocar este bloco no tabuleiro.
     *
     * @param block Block
     * @param insertedColumn int
     * @param insertedRow int
     * @return Verdadeiro ou falso (Boolean)
     */
    public boolean checkBlockPlacement(Block block, int insertedColumn, int insertedRow) {
        int letterIndex = insertedColumn;
                ...
                for (int col = 0; col < squaresSorted.size(); col++) {
                    if (block.compareColumns(currentColumnValues, (col + 1))) {
                        ...
                        if (currentColumnIndex != null) {
                            if (block.getLeftSquare(block.getSquares()) <= 1) {
                                if (maxRowSquare.getY() - insertedRow > 0) {
                                    return true;
                                } else {
                                    if (this.boardPositions[((insertedRow - 1) - (currentSquareValues.get(currentColumnIndex).getY() - 1))][(letterIndex + currentSquareValues.get(currentColumnIndex).getX() - 1)] == "#") {
                                        return true;
                                    }
                                }
                            } else {
                                if ((insertedRow - 2) + maxRowSquare.getY() > 8) {
                                    return true;
                                } else {
                                    if (this.boardPositions[((insertedRow - 1) - (currentSquareValues.get(currentColumnIndex).getY() - maxRowSquare.getY()))][(letterIndex + currentSquareValues.get(currentColumnIndex).getX() - 1)] == "#") {
                                        return true;
                                    }
                                }
                            }
     ...
     * 
     */
    public void addBlock(Block block, int insertedColumn, int insertedRow) throws PlacingBlockException {
        if (!checkBlockPlacement(block, insertedColumn, insertedRow)) {
            int letterIndex = insertedColumn;/*this.header.indexOf(insertedColumn);*/
            Square maxColumnSquare = Collections.max(block.getSquares(), Comparator.comparing(p -> p.getX()));
            Square maxRowSquare = Collections.max(block.getSquares(), Comparator.comparing(p -> p.getY()));
            Integer currentColumnIndex;
            if ((8 - (letterIndex - 1)) >= maxColumnSquare.getX()) {
                    ...
                    for (int col = 0; col < squaresSorted.size(); col++) {
                        if (block.compareColumns(currentColumnValues, (col + 1))) {
                            ...
                            if (currentColumnIndex != null) {
                                ...
                                throw new PlacingBlockException(ErrorCode.CANT_PLACE_BLOCK);
                            }
                        }
                    }
                }
