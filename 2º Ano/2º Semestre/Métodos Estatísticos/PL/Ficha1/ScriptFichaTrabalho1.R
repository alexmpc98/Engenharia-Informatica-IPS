...
prop.table(seminario$participantes)*100 #No R, a função prop.table() gera os percentuais para uma tabela
...
numSummary(obesidade[,"FAVC", drop=FALSE], statistics=c("mean", 
                                                         "sd", "IQR", "quantiles"), quantiles=c(0,.25,.5,.75,1))

# Fumar - Se fuma
Desc(obesidade$Fumar)
summary(obesidade$Fumar)
numSummary(obesidade[,"Fumar", drop=FALSE], statistics=c("mean", 
                                                         "sd", "IQR", "quantiles"), quantiles=c(0,.25,.5,.75,1))
