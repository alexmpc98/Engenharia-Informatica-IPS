...
public class Main extends Thread {
    ...
    void modifyKnapsack(String threadName) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int u = lastIndex; u < n; u++) {
                ...
                upperBound = calcUpperBound(knapsackSort, bagLower, bestWeight, lastIndex);
                if (upperBound >= calcValue(knapsackSort, lowerBound)) {
                    if (calcValue(knapsackSort, temporaryKnapsack) > calcValue(knapsackSort, lowerBound)) {
    ...
    public int calcUpperBound(ArrayList<KnapsackSort> knapsackSort, ArrayList<Integer> bagUpper, int bestWeight, int n) {
        ...
        int weightMaxCalc = bestWeight - weightSumUpperTemporary - weightSumUpper;
        int valueSum = calcValue(knapsackSort, bagUpper);
        int leftCalc = (int) (weightMaxCalc * ((double)knapsackSort.get((cUpper)).knapsackSortValues / (double)knapsackSort.get((cUpper)).knapsackSortWeights));
        int rightCalc = (int) ((double)knapsackSort.get(cUpper-1).knapsackSortValues - ((double)knapsackSort.get(cUpper-1).knapsackSortWeights - weightMaxCalc) * ((double)knapsackSort.get(cUpper - 2).knapsackSortValues / (double)knapsackSort.get(cUpper - 2).knapsackSortWeights));
        /*System.out.println(knapsackSort.get((cUpper - 2)).knapsackSortValues + "     " + knapsackSort.get((cUpper - 2)).knapsackSortWeights);
        System.out.println(knapsackSort.get(cUpper).knapsackSortValues + "     " + knapsackSort.get(cUpper).knapsackSortWeights);
        System.out.println(knapsackSort.get(cUpper-1).knapsackSortValues + "     " + knapsackSort.get(cUpper-1).knapsackSortWeights);
        System.out.println(valueSum + sigmaSum + leftCalc   + "     " + valueSum + sigmaSum+ rightCalc );
        System.out.println(leftCalc   + "     " + rightCalc);
        System.out.println(bestWeight + " - " + weightSumUpper + " - " + bagUpper + " - " + weightSumUpperTemporary + " - " + temporaryBagUpper + " - " + weightMaxCalc + " - " + leftCalc + " - " + rightCalc);*/
        int result = Math.max(valueSum + sigmaSum + leftCalc, valueSum + sigmaSum + rightCalc);
        System.out.println(bestWeight + " - " + weightSumUpper + " - " + bagUpper + " - " + weightSumUpperTemporary + " - " + temporaryBagUpper + " - " + weightMaxCalc + " - " + leftCalc + " - " + rightCalc);
        System.out.println("UB: " + weightSumUpper + " - " + cUpper + " - " + bagUpper + " - " +  result);
        return result;
    }
